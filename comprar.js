...
/*como tenemos el id del producto, queremos su nombre para el mensaje, pues tenemos que buscarlo llamando a la api y que nos traiga su registro y coger su titulo*/
        const respuesta = await fetch(`https://api.themoviedb.org/3/movie/${idPeli}?api_key=1dc5c454e07f67feeebe5884fc14b420&language=es-MX`);
...
var precio=0;//esta variable es para actualziarla en el mensaje y decir cuánto le hemos cobrado en el correo
...
btn.addEventListener('click', async function(event) {
    ...
    for(let i=0;i<prepararPedido.length;i++){
        ...
        precio+=parseInt(prepararPedido[i][1]);//aqui actualizamos el precio en cada interacción
   ...
   emailjs.send("service_o6x5yhr","template_2hju27i",{
    ...
    precio=0;//aqui reseteamos el precio
    comprar();
});
